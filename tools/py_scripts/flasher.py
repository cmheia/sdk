...
class w600dl(object):
    def __init__(self, image='', port=''):
        if os.path.isfile(image):
            ...
            if not is_serial_avaliable(port):
                raise NameError('port {} NOT exist'.format(port))

            self._log.warning(
                'open {} with {}, {}'.format(port, INIT_BAUD, INIT_TIMEOUT))
    ...
    def set_speed(self, baud=2000000):
    ...
    def info(self):
        return (self._ser.port, self._image)
    ...
    def _check_baud(self, baud=2000000, retry=16):
    ...
    def sync_to_download(self, baud=2000000, retry=16 + len('secboot running V3.1...')):
        ...
        while synced is False and 0 < c_retry:
            ...
            if state is 0:  # start
                ...
                if self.set_speed(baud) is True:
                    self._log.warning('change baud to {}'.format(baud))
                    time.sleep(0.5)
                    self.set_baudrate(baud)
                    # synced = self._check_baud(baud, retry)
                synced = True
            c_retry -= 1
        if not synced:
