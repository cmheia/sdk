...
static inline bool alg_napt_table_is_full(void)
{
    ...
    if ((napt_table_4tcp.cnt + napt_table_4udp.cnt + napt_table_4ic.cnt)
        >= NAPT_TABLE_SIZE_MAX) {
#        ifdef NAPT_ALLOC_DEBUG
        printf("@@@ napt batle: limit is reached for tcp/udp.\r\n");
#        endif
        LWIP_DEBUGF(NAPT_DEBUG,
                    ("napt batle: limit is reached for tcp/udp.\n"));
        is_full = true;
    }
...
again:
    ...
    for (udp_pcb = udp_pcbs; udp_pcb != NULL; udp_pcb = udp_pcb->next) {
        if (udp_pcb->local_port == napt_curr_port) {
            if (++cnt
                > (NAPT_LOCAL_PORT_RANGE_END - NAPT_LOCAL_PORT_RANGE_START)) {
                return 0;
            }
            goto again;
        }
    ...
    for (i = 0; i < NUM_TCP_PCB_LISTS; i++) {
        for (tcp_pcb = *tcp_pcb_lists[i]; tcp_pcb != NULL;
             tcp_pcb = tcp_pcb->next) {
            if (tcp_pcb->local_port == napt_curr_port) {
                if (++cnt > (NAPT_LOCAL_PORT_RANGE_END
                             - NAPT_LOCAL_PORT_RANGE_START)) {
                    return 0;
                }
                goto again;
            }
...
*****************************************************************************/
static inline struct napt_addr_4tu *alg_napt_get_tcp_port_by_src(u16 port,
                                                                 u8  ip)
{
    struct napt_addr_4tu *ret = NULL;
    struct napt_addr_4tu *napt;
...
*****************************************************************************/
static inline struct napt_addr_4tu *alg_napt_table_insert_4tcp(u16 src_port,
                                                               u8  ip)
{
    u16                   new_port;
    struct napt_addr_4tu *napt;
...
*****************************************************************************/
static inline struct napt_addr_4tu *alg_napt_get_udp_port_by_src(u16 port,
                                                                 u8  ip)
{
    struct napt_addr_4tu *ret = NULL;
    struct napt_addr_4tu *napt;
...
*****************************************************************************/
static inline struct napt_addr_4tu *alg_napt_table_insert_4udp(u16 src_port,
                                                               u8  ip)
{
    u16                   new_port;
    struct napt_addr_4tu *napt;
...
#    endif
    /* skip the first item */
    for (napt4ic_prev = napt_table_4ic.next; NULL != napt4ic_prev;
...
#    endif
    for (napt4tcp_prev = napt_table_4tcp.next; NULL != napt4tcp_prev;
...
#    endif
    for (napt4udp_prev = napt_table_4udp.next; NULL != napt4udp_prev;
...
/*****************************************************************************
 Prototype    : alg_napt_check_entry
...
*****************************************************************************/
static void alg_napt_check_entry(napt_tmr_type_t type)
{
    switch (type) {
...
/**
 * Call alg_napt_check_entry for each entry in dns_table - check all entries.
 */
static void alg_napt_check_tables(void)
{
    napt_tmr_type_t type;

    for (type = 0; type < NAPT_TMR_TYPE_MAX; type++) {
        alg_napt_check_entry(type);
    }
}
...
void alg_tmr(void)
{
  LWIP_DEBUGF(NAPT_DEBUG, ("alg_tmr: alg_napt_check_tables\n"));
  alg_napt_check_tables();
}
...
*****************************************************************************/
static inline u16 alg_tcpudphdr_chksum(u32        src_addr,
                                       u32        dst_addr,
                                       u8         proto,
                                       const u16 *buff,
                                       u16        len)
...
static inline int alg_output(struct netif *netif, const struct ip_hdr *ip_hdr)
{
    ...
    err = netif->output_fwd(netif, pBuf, &ipaddr);
    if (0 != err) {
        pbuf_free(pBuf);
    }
...
*****************************************************************************/
static int
alg_icmp_proc(const u8 *bssid, struct ip_hdr *ip_hdr, u8 *ehdr, u16 eth_len)
{
    int                   err;
    struct napt_addr_4ic *napt;
...
#    endif
        ...
        if (NULL == napt) {
            napt = alg_napt_table_insert_4ic(icmp_hdr->id,
                                             ip_hdr->src.addr >> 24);
            if (NULL == napt) {
#    ifdef NAPT_TABLE_MUTEX_LOCK
                sys_mutex_unlock(napt_table_lock_4ic);
...
#    endif
        ...
        icmp_hdr->chksum = 0;
        icmp_hdr->chksum = alg_iphdr_chksum((u16 *) icmp_hdr,
...
#    endif
        ...
        if (NULL != napt) {
            ...
            icmp_hdr->chksum = 0;
            icmp_hdr->chksum = alg_iphdr_chksum(
                (u16 *) icmp_hdr, ntohs(ip_hdr->_len) - iphdr_len);

            ip_hdr->dest.addr
                = ((napt->src_ip) << 24)
                  | (ip_addr_get_ip4_u32(&net_if->next->ip_addr) & 0x00ffffff);
            ip_hdr->_chksum = 0;
            ip_hdr->_chksum = alg_iphdr_chksum((u16 *) ip_hdr, iphdr_len);
...
*****************************************************************************/
static int
alg_tcp_proc(const u8 *bssid, struct ip_hdr *ip_hdr, u8 *ehdr, u16 eth_len)
{
    int                   err;
    u8                    src_ip;
    struct napt_addr_4tu *napt;
...
#    endif
        tcp_hdr->chksum = 0;
        tcp_hdr->chksum
            = alg_tcpudphdr_chksum(ip_hdr->src.addr,
                                   ip_hdr->dest.addr,
                                   IP_PROTO_TCP,
                                   (u16 *) tcp_hdr,
                                   ntohs(ip_hdr->_len) - iphdr_len);
...
*****************************************************************************/
static int
alg_udp_proc(const u8 *bssid, struct ip_hdr *ip_hdr, u8 *ehdr, u16 eth_len)
{
    int                   err = 0;
    u8                    src_ip;
    struct napt_addr_4tu *napt;
    ...
    redo:
        if (is_dns) {
            ...
            if ((2 == is_dns) && ip_addr_isany(dns))
                goto end;
            ip_hdr->dest.addr = ip_addr_get_ip4_u32(dns);
        }
        ip_hdr->src.addr = ip_addr_get_ip4_u32(&net_if->ip_addr);
        ip_hdr->_chksum  = 0;
        ip_hdr->_chksum  = alg_iphdr_chksum((u16 *) ip_hdr, iphdr_len);
...
*****************************************************************************/
static int
alg_gre_proc(const u8 *bssid, struct ip_hdr *ip_hdr, u8 *ehdr, u16 eth_len)
{
    int           err;
    u8            src_ip;
    u8            iphdr_len;
